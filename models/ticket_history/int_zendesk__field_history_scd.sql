...
-- we need to backfill to persist values that have been previously updated and are still valid
    select
        ...
        {% for col in ticket_columns if col.name|lower not in ['date_day','ending_day','source_relation', 'ticket_id','ticket_day_id'] %}

        ,{{ col.name }}
        ,sum(case when {{ col.name }} is null
                then 0
                else 1
                    end) over (order by source_relation, ticket_id, date_day rows unbounded preceding) as {{ col.name }}_field_partition
        {% endfor %}
